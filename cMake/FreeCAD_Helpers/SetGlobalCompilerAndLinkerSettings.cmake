macro(SetGlobalCompilerAndLinkerSettings)
    ...
    option(FREECAD_USE_LTO "Enable interprocedural optimization (LTO/IPO) for release builds" OFF)
    if(FREECAD_USE_LTO)
        include(CheckIPOSupported)
        check_ipo_supported(RESULT FREECAD_IPO_SUPPORTED OUTPUT FREECAD_IPO_ERROR)
        if(FREECAD_IPO_SUPPORTED)
            set(CMAKE_INTERPROCEDURAL_OPTIMIZATION_RELEASE ON)
            message(STATUS "LTO/IPO enabled for Release builds")
        else()
            message(WARNING "FREECAD_USE_LTO is ON but IPO is not supported: ${FREECAD_IPO_ERROR}")
        endif()
    endif()
    ...
    if(APPLE)
        ...
        if(NOT CMAKE_OSX_ARCHITECTURES AND CMAKE_SYSTEM_PROCESSOR STREQUAL "arm64")
            ...
            message(STATUS "Pinning CMAKE_OSX_ARCHITECTURES=arm64 for Apple Silicon")
        endif()
        option(FREECAD_APPLE_SILICON_TUNING "Add -mcpu=apple-m1 for Apple Silicon release builds" OFF)
        if(FREECAD_APPLE_SILICON_TUNING AND CMAKE_SYSTEM_PROCESSOR STREQUAL "arm64")
            add_compile_options($<$<AND:$<COMPILE_LANGUAGE:C,CXX>,$<CONFIG:Release>>:-mcpu=apple-m1>)
            message(STATUS "Apple Silicon CPU tuning: -mcpu=apple-m1")
        endif()
    endif()
